...
def persist_data(rows, aggregate, items):
    ...
    aggregate["pstmo_diagnostics"] = {
        ...
        for key in (
            "runtime_s", "los_runtime_s", "raw_input_points", "conditioning_output_points",
            "los_output_points", "los_attempted_shortcuts", "los_accepted_shortcuts",
            "los_safety_rejections", "evaluations", "coarse_shape_evaluations",
...
def plot_case_composite(item, rows_for_case):
    ...
    screenshot = np.asarray(Image.open(ROOT / item["rviz_screenshot"]).convert("RGB"))
